...
public class TheoDoiDonHang extends AppCompatActivity {
    ...
    private TextView txt_tieude_acti, txt_madon_tddh, txt_thoigiandat_tddh, txt_thoigiannhan_tddh, txt_tennguoinhan_tddh, txt_SDT_tddh, txt_diachi_tddh, txt_tamtinh_tddh, txt_phiship_tddh, txt_giamgia_tddh, txt_tongtien_tddh, txt_dando_theodoidonhang;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        txt_thoigiandat_tddh = findViewById(R.id.txt_thoigiandat_tddh);
        txt_thoigiannhan_tddh = findViewById(R.id.txt_thoigiannhan_tddh);
        ...
        final ViewGroup.LayoutParams params_btn_danhgia_sanpham = btn_danhgia_sanpham.getLayoutParams();
        height_area_danhgia = params_btn_danhgia_sanpham.height;
        params_btn_danhgia_sanpham.height = 0;
        btn_danhgia_sanpham.setLayoutParams(params_btn_danhgia_sanpham);


        txt_tieude_acti.setText("Chi tiết đơn hàng");
        btn_back.setImageResource(R.drawable.ic_close_black);
            ...
            public void onEvent(@Nullable DocumentSnapshot snapshot, @Nullable FirebaseFirestoreException e) {
                ...
                if (snapshot != null && snapshot.exists()) {
                    ...
                    for (int i = 0; i < bill_key.length; i++)
                    {
                        ...
                        if (bill_key[i].toString().equals("thoi_gian_dat")) txt_thoigiandat_tddh.setText(bill_value[i].toString());
                        if (bill_key[i].toString().equals("thoi_gian_nhan"))
                        {
                            if (!bill_value[i].toString().equals(""))
                            {
                                txt_thoigiannhan_tddh.setText(bill_value[i].toString());
                            }
                            else txt_thoigiannhan_tddh.setText("");
                        }
                    ...
                    else
                    {
                        ...
                        params_btn_danhgia_sanpham.height = height_area_danhgia;
                        btn_danhgia_sanpham.setLayoutParams(params_btn_danhgia_sanpham);
                    }
                    reload = true;
                ...
                btn_save_danhgia.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (add)
                        {
                            ...
                            for (int i = 0; i < ds_rating.size(); i++)
                            {
                                final String danhgia = ds_rating.get(i).getDanhgia();
                                //update bill
                                ...
                                doc.update("danh_gia", danhgia).addOnSuccessListener(new OnSuccessListener<Void>() {
                                    @Override
                                ...
                                //update food info
                                final DocumentReference DocRef = database.collection("Food").document(ds_rating.get(i).getTenmon());
                                database.runTransaction(new Transaction.Function<Void>() {
                                    @Override
                                    public Void apply(Transaction transaction) throws FirebaseFirestoreException {
                                        DocumentSnapshot snapshot = transaction.get(DocRef);

                                        if (danhgia.equals("danhgiatot"))
                                        {
                                            int new_so_luot_danh_gia_tot = Integer.parseInt(snapshot.getString("so_luot_danh_gia_tot")) + 1;
                                            transaction.update(DocRef, "so_luot_danh_gia_tot", String.valueOf(new_so_luot_danh_gia_tot));
                                        }
                                        else {
                                            int new_so_luot_danh_gia_khong_tot = Integer.parseInt(snapshot.getString("so_luot_danh_gia_khong_tot")) + 1;
                                            transaction.update(DocRef, "so_luot_danh_gia_khong_tot", String.valueOf(new_so_luot_danh_gia_khong_tot));
                                        }
    ...
    void update_user_info_bill()
    {
        ...
        database.runTransaction(new Transaction.Function<Void>() {
            ...
            @Override
            public void onSuccess(Void aVoid) {
                Toast.makeText(TheoDoiDonHang.this, "bbb", Toast.LENGTH_SHORT).show();
            }
        })
        .addOnFailureListener(new OnFailureListener() {
            @Override
    ...
    void update_food_info()
    {
        for (int j = 0; j < Mang_DanhSachMonAnKhachChon.size(); j++)
        {
            final DocumentReference DocRef = database.collection("Food").document(Mang_DanhSachMonAnKhachChon.get(j).getTenmon());
            database.runTransaction(new Transaction.Function<Void>() {
                @Override
                public Void apply(Transaction transaction) throws FirebaseFirestoreException {
                    DocumentSnapshot snapshot = transaction.get(DocRef);
                ...
                @Override
                public void onSuccess(Void aVoid) {
                    Toast.makeText(TheoDoiDonHang.this, "aa", Toast.LENGTH_SHORT).show();
                }
            })
            .addOnFailureListener(new OnFailureListener() {
                @Override
